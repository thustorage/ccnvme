...
static struct dentry *ext4_mount(struct file_system_type *fs_type, int flags, const char *dev_name,
                                 void *data);
...
MODULE_ALIAS_FS("mqfs-ext2");
MODULE_ALIAS("mqfs-ext2");
...
MODULE_ALIAS_FS("mqfs-ext3");
MODULE_ALIAS("mqfs-ext3");
...
static int ext4_superblock_csum_verify(struct super_block *sb, struct ext4_super_block *es) {
    if (!ext4_has_metadata_csum(sb)) return 1;
...
void ext4_superblock_csum_set(struct super_block *sb) {
    ...
    if (!ext4_has_metadata_csum(sb)) return;
...
void *ext4_kvmalloc(size_t size, gfp_t flags) {
    ...
    if (!ret) ret = __vmalloc(size, flags, PAGE_KERNEL);
    return ret;
}
...
void *ext4_kvzalloc(size_t size, gfp_t flags) {
    ...
    if (!ret) ret = __vmalloc(size, flags | __GFP_ZERO, PAGE_KERNEL);
    return ret;
}
...
__u32 ext4_itable_unused_count(struct super_block *sb, struct ext4_group_desc *bg) {
    return le16_to_cpu(bg->bg_itable_unused_lo) |
           (EXT4_DESC_SIZE(sb) >= EXT4_MIN_DESC_SIZE_64BIT
                ? (__u32)le16_to_cpu(bg->bg_itable_unused_hi) << 16
                : 0);
}
...
static void __save_error_info(struct super_block *sb, const char *func, unsigned int line) {
    ...
    if (bdev_read_only(sb->s_bdev)) return;
     ...
     * started already
     */
    if (!es->s_error_count) mod_timer(&EXT4_SB(sb)->s_err_report, jiffies + 24 * 60 * 60 * HZ);
...
static void ext4_handle_error(struct super_block *sb) {
    if (test_opt(sb, WARN_ON_ERROR)) WARN_ON_ONCE(1);
    ...
    if (!test_opt(sb, ERRORS_CONT)) {
        ...
        if (journal) jbd2_journal_abort(journal, -EIO);
    }
    ...
    if (test_opt(sb, ERRORS_PANIC)) {
        if (EXT4_SB(sb)->s_journal && !(EXT4_SB(sb)->s_journal->j_flags & JBD2_REC_ERR)) return;
        panic("EXT4-fs (device %s): panic forced after error\n", sb->s_id);
    }
}

#define ext4_error_ratelimit(sb) \
    ___ratelimit(&(EXT4_SB(sb)->s_err_ratelimit_state), "EXT4-fs error")
...
void __ext4_error_inode(struct inode *inode, const char *function, unsigned int line,
                        ext4_fsblk_t block, const char *fmt, ...) {
    va_list args;
    struct va_format vaf;
...
void __ext4_error_file(struct file *file, const char *function, unsigned int line,
                       ext4_fsblk_t block, const char *fmt, ...) {
    va_list args;
    struct va_format vaf;
    ...
    if (ext4_error_ratelimit(inode->i_sb)) {
        ...
        if (block)
            printk(KERN_CRIT
                   ...
                   inode->i_sb->s_id, function, line, inode->i_ino, block, current->comm, path,
                   &vaf);
        else
            printk(KERN_CRIT
                   "EXT4-fs error (device %s): %s:%d: inode #%lu: "
    ...
    switch (errno) {
        ...
        default:
            ...
            if (nbuf) {
                ...
                if (snprintf(nbuf, 16, "error %d", -errno) >= 0) errstr = nbuf;
            }
            break;
    }
...
void __ext4_std_error(struct super_block *sb, const char *function, unsigned int line, int errno) {
     ...
     * an error. */
    if (errno == -EROFS && journal_current_handle() == NULL && sb_rdonly(sb)) return;
...
void __ext4_abort(struct super_block *sb, const char *function, unsigned int line, const char *fmt,
    ...
    if (sb_rdonly(sb) == 0) {
        ...
        smp_wmb();
        sb->s_flags |= SB_RDONLY;
        if (EXT4_SB(sb)->s_journal) jbd2_journal_abort(EXT4_SB(sb)->s_journal, -EIO);
        save_error_info(sb, function, line);
    }
    if (test_opt(sb, ERRORS_PANIC)) {
        if (EXT4_SB(sb)->s_journal && !(EXT4_SB(sb)->s_journal->j_flags & JBD2_REC_ERR)) return;
...
void __ext4_msg(struct super_block *sb, const char *prefix, const char *fmt, ...) {
    ...
    if (!___ratelimit(&(EXT4_SB(sb)->s_msg_ratelimit_state), "EXT4-fs")) return;
...
#define ext4_warning_ratelimit(sb) \
    ___ratelimit(&(EXT4_SB(sb)->s_warning_ratelimit_state), "EXT4-fs warning")

void __ext4_warning(struct super_block *sb, const char *function, unsigned int line,
                    const char *fmt, ...) {
    struct va_format vaf;
    va_list args;

    if (!ext4_warning_ratelimit(sb)) return;
...
void __ext4_warning_inode(const struct inode *inode, const char *function, unsigned int line,
                          const char *fmt, ...) {
    struct va_format vaf;
    va_list args;

    if (!ext4_warning_ratelimit(inode->i_sb)) return;
...
void __ext4_grp_locked_error(const char *function, unsigned int line, struct super_block *sb,
                             ext4_group_t grp, unsigned long ino, ext4_fsblk_t block,
                             const char *fmt, ...) __releases(bitlock) __acquires(bitlock) {
    struct va_format vaf;
    va_list args;
    struct ext4_super_block *es = EXT4_SB(sb)->s_es;
    ...
    if (ext4_error_ratelimit(sb)) {
        ...
        vaf.fmt = fmt;
        vaf.va = &args;
        printk(KERN_CRIT "EXT4-fs error (device %s): %s:%d: group %u, ", sb->s_id, function, line,
               grp);
        if (ino) printk(KERN_CONT "inode %lu: ", ino);
        if (block) printk(KERN_CONT "block %llu:", (unsigned long long)block);
        printk(KERN_CONT "%pV\n", &vaf);
        va_end(args);
    }
...
void ext4_mark_group_bitmap_corrupted(struct super_block *sb, ext4_group_t group,
                                      unsigned int flags) {
    struct ext4_sb_info *sbi = EXT4_SB(sb);
    ...
    if (flags & EXT4_GROUP_INFO_BBITMAP_CORRUPT) {
        ret = ext4_test_and_set_bit(EXT4_GROUP_INFO_BBITMAP_CORRUPT_BIT, &grp->bb_state);
        if (!ret) percpu_counter_sub(&sbi->s_freeclusters_counter, grp->bb_free);
    }

    if (flags & EXT4_GROUP_INFO_IBITMAP_CORRUPT) {
        ret = ext4_test_and_set_bit(EXT4_GROUP_INFO_IBITMAP_CORRUPT_BIT, &grp->bb_state);
        if (!ret && gdp) {
            int count;
...
fail:
    ext4_msg(sb, KERN_ERR, "failed to open journal device %s: %ld", __bdevname(dev, b),
             PTR_ERR(bdev));
    return NULL;
}
...
static void dump_orphan_list(struct super_block *sb, struct ext4_sb_info *sbi) {
    ...
    list_for_each(l, &sbi->s_orphan) {
        ...
        printk(KERN_ERR
               ...
               inode->i_sb->s_id, inode->i_ino, inode, inode->i_mode, inode->i_nlink,
               NEXT_ORPHAN(inode));
    }
}
...
static inline void ext4_quota_off_umount(struct super_block *sb) {
    ...
    for (type = 0; type < EXT4_MAXQUOTAS; type++) ext4_quota_off(sb, type);
}
 ...
 */
static inline char *get_qf_name(struct super_block *sb, struct ext4_sb_info *sbi, int type) {
    return rcu_dereference_protected(sbi->s_qf_names[type], lockdep_is_held(&sb->s_umount));
}
#else
static inline void ext4_quota_off_umount(struct super_block *sb) {}
#endif
...
static void ext4_put_super(struct super_block *sb) {
    ...
    // for mqfs
    graceful_shutdown(sbi->streams, sbi->nr_streams, sb);
    ...
    if (sbi->s_journal) {
        ...
        sbi->s_journal = NULL;
        if ((err < 0) && !aborted) ext4_abort(sb, "Couldn't clean up the journal");
    }
    ...
    if (!sb_rdonly(sb)) ext4_commit_super(sb, 1);

    for (i = 0; i < sbi->s_gdb_count; i++) brelse(sbi->s_group_desc[i]);
...
#ifdef CONFIG_QUOTA
    for (i = 0; i < EXT4_MAXQUOTAS; i++) kfree(get_qf_name(sb, sbi, i));
#endif
    ...
    if (!list_empty(&sbi->s_orphan)) dump_orphan_list(sb, sbi);
    ...
    if (sbi->s_mmp_tsk) kthread_stop(sbi->s_mmp_tsk);
    brelse(sbi->s_sbh);
    sb->s_fs_info = NULL;
    /*
    ...
    // for mqfs
    olayer_unregister_streams(sbi->nr_streams, sbi->streams, 1, &sbi->s_sb->s_bdev);
    sbi->nr_streams = 0;
    kfree(sbi->streams);
...
static struct inode *ext4_alloc_inode(struct super_block *sb) {
    ...
    // for mqfs
    INIT_LIST_HEAD(&ei->fsmeta_pending_list);
    INIT_LIST_HEAD(&ei->inodemeta_pending_list);
    INIT_LIST_HEAD(&ei->copyout_inode_list);
    return &ei->vfs_inode;
}
...
static void ext4_destroy_inode(struct inode *inode) {
    if (!list_empty(&(EXT4_I(inode)->i_orphan))) {
        ext4_msg(inode->i_sb, KERN_ERR, "Inode %lu (%p): orphan list check failed!", inode->i_ino,
                 EXT4_I(inode));
        print_hex_dump(KERN_INFO, "", DUMP_PREFIX_ADDRESS, 16, 4, EXT4_I(inode),
                       sizeof(struct ext4_inode_info), true);
        dump_stack();
    }
...
static int __init init_inodecache(void) {
    ext4_inode_cachep =
        kmem_cache_create_usercopy("mqfs_inode_cache", sizeof(struct ext4_inode_info), 0,
                                   (SLAB_RECLAIM_ACCOUNT | SLAB_MEM_SPREAD | SLAB_ACCOUNT),
                                   offsetof(struct ext4_inode_info, i_data),
                                   sizeof_field(struct ext4_inode_info, i_data), init_once);
    if (ext4_inode_cachep == NULL) return -ENOMEM;
    return 0;
}
...
static struct inode *ext4_nfs_get_inode(struct super_block *sb, u64 ino, u32 generation) {
    ...
    if (ino < EXT4_FIRST_INO(sb) && ino != EXT4_ROOT_INO) return ERR_PTR(-ESTALE);
    if (ino > le32_to_cpu(EXT4_SB(sb)->s_es->s_inodes_count)) return ERR_PTR(-ESTALE);
...
static struct dentry *ext4_fh_to_dentry(struct super_block *sb, struct fid *fid, int fh_len,
                                        int fh_type) {
    return generic_fh_to_dentry(sb, fid, fh_len, fh_type, ext4_nfs_get_inode);
}

static struct dentry *ext4_fh_to_parent(struct super_block *sb, struct fid *fid, int fh_len,
                                        int fh_type) {
...
static int ext4_get_context(struct inode *inode, void *ctx, size_t len) {
    return ext4_xattr_get(inode, EXT4_XATTR_INDEX_ENCRYPTION, EXT4_XATTR_NAME_ENCRYPTION_CONTEXT,
                          ctx, len);
}
...
static int ext4_set_context(struct inode *inode, const void *ctx, size_t len, void *fs_data) {
     ...
     */
    if (inode->i_ino == EXT4_ROOT_INO) return -EPERM;

    if (WARN_ON_ONCE(IS_DAX(inode) && i_size_read(inode))) return -EINVAL;
    ...
    if (res) return res;
retry:
    res = ext4_xattr_set_credits(inode, len, false /* is_create */, &credits);
    if (res) return res;
    ...
    if (!res) {
        ...
        if (res) EXT4_ERROR_INODE(inode, "Failed to mark inode dirty");
    }
    res2 = ext4_journal_stop(handle);

    if (res == -ENOSPC && ext4_should_retry_alloc(inode->i_sb, &retries)) goto retry;
    if (!res) res = res2;
    return res;
}
...
static ssize_t ext4_quota_read(struct super_block *sb, int type, char *data, size_t len,
                               loff_t off);
static ssize_t ext4_quota_write(struct super_block *sb, int type, const char *data, size_t len,
                                loff_t off);
...
enum {
    ...
    Opt_nombcache,
    Opt_nr_streams, // for mqfs
    Opt_crashmonkey, // for crashmonkey
};
...
static const match_table_t tokens = {
    ...
    {Opt_nr_streams, "nr_streams=%u"}, // for mqfs
    {Opt_crashmonkey, "crashmonkey=%u"}, // for mqfs
...
static ext4_fsblk_t get_sb_block(void **data) {
    ...
    if (!options || strncmp(options, "sb=", 3) != 0) return 1; /* Default location */
...
static int set_qf_name(struct super_block *sb, int qtype, substring_t *args) {
    ...
    if (old_qname) {
        if (strcmp(old_qname, qname) == 0)
            ...
            ext4_msg(sb, KERN_ERR, "%s quota file already specified", QTYPE2NAME(qtype));
        goto errout;
    }
    if (strchr(qname, '/')) {
...
static const struct mount_opts {
    ...
    {Opt_dioread_nolock, EXT4_MOUNT_DIOREAD_NOLOCK, MOPT_EXT4_ONLY | MOPT_SET},
    {Opt_dioread_lock, EXT4_MOUNT_DIOREAD_NOLOCK, MOPT_EXT4_ONLY | MOPT_CLEAR},
    {Opt_discard, EXT4_MOUNT_DISCARD, MOPT_SET},
    {Opt_nodiscard, EXT4_MOUNT_DISCARD, MOPT_CLEAR},
    {Opt_delalloc, EXT4_MOUNT_DELALLOC, MOPT_EXT4_ONLY | MOPT_SET | MOPT_EXPLICIT},
    {Opt_nodelalloc, EXT4_MOUNT_DELALLOC, MOPT_EXT4_ONLY | MOPT_CLEAR},
    {Opt_warn_on_error, EXT4_MOUNT_WARN_ON_ERROR, MOPT_SET},
    {Opt_nowarn_on_error, EXT4_MOUNT_WARN_ON_ERROR, MOPT_CLEAR},
    {Opt_nojournal_checksum, EXT4_MOUNT_JOURNAL_CHECKSUM, MOPT_EXT4_ONLY | MOPT_CLEAR},
    {Opt_journal_checksum, EXT4_MOUNT_JOURNAL_CHECKSUM, MOPT_EXT4_ONLY | MOPT_SET | MOPT_EXPLICIT},
    ...
    {Opt_data_err_abort, EXT4_MOUNT_DATA_ERR_ABORT, MOPT_NO_EXT2},
    {Opt_data_err_ignore, EXT4_MOUNT_DATA_ERR_ABORT, MOPT_NO_EXT2},
    ...
    {Opt_nr_streams, 0, MOPT_GTE0}, // for mqfs
    {Opt_crashmonkey, 0, MOPT_GTE0}, // for mqfs
    ...
    {Opt_data_writeback, EXT4_MOUNT_WRITEBACK_DATA, MOPT_NO_EXT2 | MOPT_DATAJ},
    ...
    {Opt_usrquota, EXT4_MOUNT_QUOTA | EXT4_MOUNT_USRQUOTA, MOPT_SET | MOPT_Q},
...
static int handle_mount_opt(struct super_block *sb, char *opt, int token, substring_t *args,
                            unsigned long *journal_devnum, unsigned int *journal_ioprio,
                            int is_remount) {
    struct ext4_sb_info *sbi = EXT4_SB(sb);
    ...
    if (token == Opt_err_panic && !capable(CAP_SYS_ADMIN)) {
        ext4_msg(sb, KERN_ERR, "Mount option \"%s\" not allowed for unprivileged mounts", opt);
        return -1;
    }

    if (args->from && !(m->flags & MOPT_STRING) && match_int(args, &arg)) return -1;
    if (args->from && (m->flags & MOPT_GTE0) && (arg < 0)) return -1;
    ...
    if (m->flags & MOPT_CLEAR_ERR) clear_opt(sb, ERRORS_MASK);
    ...
    if (m->flags & MOPT_NOSUPPORT) {
        ...
        if (arg == 0) arg = JBD2_DEFAULT_MAX_COMMIT_AGE;
        sbi->s_commit_interval = HZ * arg;
    } else if (token == Opt_debug_want_extra_isize) {
        sbi->s_want_extra_isize = arg;
    } else if (token == Opt_max_batch_time) {
        sbi->s_max_batch_time = arg;
    } else if (token == Opt_nr_streams) { // for mqfs
        sbi->nr_streams = arg;   
    } else if (token == Opt_crashmonkey) { // for crashmonkey test
        sbi->crashmonkey = arg;
        ...
        if (!args->from) arg = EXT4_DEF_LI_WAIT_MULT;
        ...
        *journal_devnum = arg;
        ...
        if (!capable(CAP_SYS_ADMIN) &&
            ...
            ext4_msg(sb, KERN_ERR, "error: Insufficient access to journal path %s", journal_path);
            return -1;
        }
...
static int parse_options(char *options, struct super_block *sb, unsigned long *journal_devnum,
    ...
    if (usr_qf_name || grp_qf_name) {
        if (test_opt(sb, USRQUOTA) && usr_qf_name) clear_opt(sb, USRQUOTA);

        if (test_opt(sb, GRPQUOTA) && grp_qf_name) clear_opt(sb, GRPQUOTA);
...
static inline void ext4_show_quota_options(struct seq_file *seq, struct super_block *sb) {
    ...
    if (usr_qf_name) seq_show_option(seq, "usrjquota", usr_qf_name);
    if (grp_qf_name) seq_show_option(seq, "grpjquota", grp_qf_name);
    rcu_read_unlock();
#endif
}
...
static const char *token2str(int token) {
    ...
    for (t = tokens; t->token != Opt_err; t++)
        if (t->token == token && !strchr(t->pattern, '=')) break;
    return t->pattern;
}
...
#define SEQ_OPTS_PRINT(str, arg) seq_printf(seq, "%c" str, sep, arg)
    ...
    if (sbi->s_sb_block != 1) SEQ_OPTS_PRINT("sb=%llu", sbi->s_sb_block);

    for (m = ext4_mount_opts; m->token != Opt_err; m++) {
        int want_set = m->flags & MOPT_SET;
        if (((m->flags & (MOPT_SET | MOPT_CLEAR)) == 0) || (m->flags & MOPT_CLEAR_ERR)) continue;
        if (!nodefs && !(m->mount_opt & (sbi->s_mount_opt ^ def_mount_opt)))
            continue; /* skip if same as the default */
        if ((want_set && (sbi->s_mount_opt & m->mount_opt) != m->mount_opt) ||
    ...
    if (nodefs || !uid_eq(sbi->s_resuid, make_kuid(sb->s_user_ns, EXT4_DEF_RESUID)) ||
        ...
        SEQ_OPTS_PRINT("resuid=%u", from_kuid_munged(sb->s_user_ns, sbi->s_resuid));
    if (nodefs || !gid_eq(sbi->s_resgid, make_kgid(sb->s_user_ns, EXT4_DEF_RESGID)) ||
        le16_to_cpu(es->s_def_resgid) != EXT4_DEF_RESGID)
        SEQ_OPTS_PRINT("resgid=%u", from_kgid_munged(sb->s_user_ns, sbi->s_resgid));
    def_errors = nodefs ? -1 : le16_to_cpu(es->s_errors);
    if (test_opt(sb, ERRORS_RO) && def_errors != EXT4_ERRORS_RO) SEQ_OPTS_PUTS("errors=remount-ro");
    ...
    if (sb->s_flags & SB_I_VERSION) SEQ_OPTS_PUTS("i_version");
    if (nodefs || sbi->s_stripe) SEQ_OPTS_PRINT("stripe=%lu", sbi->s_stripe);
    ...
    if (test_opt(sb, INIT_INODE_TABLE) &&
        (nodefs || (sbi->s_li_wait_mult != EXT4_DEF_LI_WAIT_MULT)))
        SEQ_OPTS_PRINT("init_itable=%u", sbi->s_li_wait_mult);
    if (nodefs || sbi->s_max_dir_size_kb)
        SEQ_OPTS_PRINT("max_dir_size_kb=%u", sbi->s_max_dir_size_kb);
    if (test_opt(sb, DATA_ERR_ABORT)) SEQ_OPTS_PUTS("data_err=abort");
    if (DUMMY_ENCRYPTION_ENABLED(sbi)) SEQ_OPTS_PUTS("test_dummy_encryption");
...
static int ext4_setup_super(struct super_block *sb, struct ext4_super_block *es, int read_only) {
    ...
    if (!(sbi->s_mount_state & EXT4_VALID_FS))
        ...
        ext4_msg(sb, KERN_WARNING,
                 ...
                 "running e2fsck is recommended");
    if (!sbi->s_journal) es->s_state &= cpu_to_le16(~EXT4_VALID_FS);
    if (!(__s16)le16_to_cpu(es->s_max_mnt_count))
        es->s_max_mnt_count = cpu_to_le16(EXT4_DFL_MAX_MNT_COUNT);
    le16_add_cpu(&es->s_mnt_count, 1);
    es->s_mtime = cpu_to_le32(get_seconds());
    ext4_update_dynamic_rev(sb);
    if (sbi->s_journal) ext4_set_feature_journal_needs_recovery(sb);
    ...
    if (test_opt(sb, DEBUG))
        printk(KERN_INFO
               ...
               EXT4_INODES_PER_GROUP(sb), sbi->s_mount_opt, sbi->s_mount_opt2);
...
int ext4_alloc_flex_bg_array(struct super_block *sb, ext4_group_t ngroup) {
    ...
    if (size <= sbi->s_flex_groups_allocated) return 0;
    ...
    if (sbi->s_flex_groups) {
        memcpy(new_groups, sbi->s_flex_groups,
               (sbi->s_flex_groups_allocated * sizeof(struct flex_groups)));
        kvfree(sbi->s_flex_groups);
    }
...
static __le16 ext4_group_desc_csum(struct super_block *sb, __u32 block_group,
    ...
    /* old crc16 code */
    if (!ext4_has_feature_gdt_csum(sb)) return 0;
...
void ext4_group_desc_csum_set(struct super_block *sb, __u32 block_group,
                              struct ext4_group_desc *gdp) {
    if (!ext4_has_group_desc_csum(sb)) return;
    gdp->bg_checksum = ext4_group_desc_csum(sb, block_group, gdp);
}

/* Called at mount-time, super-block is locked */
static int ext4_check_descriptors(struct super_block *sb, ext4_fsblk_t sb_block,
    ...
    if (ext4_has_feature_flex_bg(sb)) flexbg_flag = 1;
    ...
    for (i = 0; i < sbi->s_groups_count; i++) {
        ...
        if (!ext4_group_desc_csum_verify(sb, i, gdp)) {
            ext4_msg(sb, KERN_ERR,
                     ...
                     i, le16_to_cpu(ext4_group_desc_csum(sb, i, gdp)),
        ...
        ext4_unlock_group(sb, i);
        if (!flexbg_flag) first_block += EXT4_BLOCKS_PER_GROUP(sb);
    }
    if (NULL != first_not_zeroed) *first_not_zeroed = grp;
    return 1;
}
...
static void ext4_orphan_cleanup(struct super_block *sb, struct ext4_super_block *es) {
    ...
    while (es->s_last_orphan) {
        ...
        if (inode->i_nlink) {
            if (test_opt(sb, DEBUG))
                ext4_msg(sb, KERN_DEBUG, "%s: truncating inode %lu to %lld bytes", __func__,
                         inode->i_ino, inode->i_size);
            jbd_debug(2, "truncating inode %lu to %lld bytes\n", inode->i_ino, inode->i_size);
            inode_lock(inode);
            truncate_inode_pages(inode->i_mapping, inode->i_size);
            ret = ext4_truncate(inode);
            if (ret) ext4_std_error(inode->i_sb, ret);
            inode_unlock(inode);
            nr_truncates++;
        } else {
            if (test_opt(sb, DEBUG))
                ext4_msg(sb, KERN_DEBUG, "%s: deleting unreferenced inode %lu", __func__,
                         inode->i_ino);
...
#define PLURAL(x) (x), ((x) == 1) ? "" : "s"
    ...
    if (nr_orphans) ext4_msg(sb, KERN_INFO, "%d orphan inode%s deleted", PLURAL(nr_orphans));
    if (nr_truncates) ext4_msg(sb, KERN_INFO, "%d truncate%s cleaned up", PLURAL(nr_truncates));
#ifdef CONFIG_QUOTA
    /* Turn off quotas if they were enabled for orphan cleanup */
    if (quota_update) {
        for (i = 0; i < EXT4_MAXQUOTAS; i++) {
            if (sb_dqopt(sb)->files[i]) dquot_quota_off(sb, i);
        }
    }
#endif
...
static loff_t ext4_max_size(int blkbits, int has_huge_files) {
    ...
    if (res > upper_limit) res = upper_limit;
...
static ext4_fsblk_t descriptor_loc(struct super_block *sb, ext4_fsblk_t logical_sb_block, int nr) {
    ...
    if (ext4_bg_has_super(sb, bg)) has_super = 1;
...
static int ext4_feature_set_ok(struct super_block *sb, int readonly) {
    if (ext4_has_unknown_ext4_incompat_features(sb)) {
        ext4_msg(
            ...
            (le32_to_cpu(EXT4_SB(sb)->s_es->s_feature_incompat) & ~EXT4_FEATURE_INCOMPAT_SUPP));
        return 0;
    }
...
static void print_daily_error_info(struct timer_list *t) {
    ...
    if (es->s_first_error_time) {
        printk(KERN_NOTICE "EXT4-fs (%s): initial error at time %u: %.*s:%d", sb->s_id,
               le32_to_cpu(es->s_first_error_time), (int)sizeof(es->s_first_error_func),
               es->s_first_error_func, le32_to_cpu(es->s_first_error_line));
        if (es->s_first_error_ino)
            printk(KERN_CONT ": inode %u", le32_to_cpu(es->s_first_error_ino));
        if (es->s_first_error_block)
            printk(KERN_CONT ": block %llu",
                   (unsigned long long)le64_to_cpu(es->s_first_error_block));
        printk(KERN_CONT "\n");
    }
    if (es->s_last_error_time) {
        printk(KERN_NOTICE "EXT4-fs (%s): last error at time %u: %.*s:%d", sb->s_id,
               le32_to_cpu(es->s_last_error_time), (int)sizeof(es->s_last_error_func),
               es->s_last_error_func, le32_to_cpu(es->s_last_error_line));
        if (es->s_last_error_ino) printk(KERN_CONT ": inode %u", le32_to_cpu(es->s_last_error_ino));
        if (es->s_last_error_block)
            printk(KERN_CONT ": block %llu",
                   (unsigned long long)le64_to_cpu(es->s_last_error_block));
        printk(KERN_CONT "\n");
    }
    ...
    while (true) {
        ...
        list_for_each_safe(pos, n, &eli->li_request_list) {
            ...
            if (time_before(jiffies, elr->lr_next_sched)) {
                if (time_before(elr->lr_next_sched, next_wakeup)) next_wakeup = elr->lr_next_sched;
                continue;
            }
...
int ext4_register_li_request(struct super_block *sb, ext4_group_t first_not_zeroed) {
    ...
    if (first_not_zeroed == ngroups || sb_rdonly(sb) || !test_opt(sb, INIT_INODE_TABLE)) goto out;
...
static int set_journal_csum_feature_set(struct super_block *sb) {
    ...
    jbd2_journal_clear_features(sbi->s_journal, JBD2_FEATURE_COMPAT_CHECKSUM, 0,
...
static int count_overhead(struct super_block *sb, ext4_group_t grp, char *buf) {
    ...
    for (i = 0; i < ngroups; i++) {
        ...
        count += j;
        for (; j > 0; j--) ext4_set_bit(EXT4_B2C(sbi, s++), buf);
    }
    if (!count) return 0;
...
int ext4_calculate_overhead(struct super_block *sb) {
    ...
    for (i = 0; i < ngroups; i++) {
        ...
        overhead += blks;
        if (blks) memset(buf, 0, PAGE_SIZE);
        cond_resched();
    }
...
static void ext4_set_resv_clusters(struct super_block *sb) {
     ...
     */
    if (!ext4_has_feature_extents(sb)) return;
    /*
...
static int ext4_fill_super(struct super_block *sb, void *data, int silent) {
    ...
    if (!sbi->s_blockgroup_lock) goto out_free_base;
    ...
    if (sb->s_magic != EXT4_SUPER_MAGIC) goto cantfind_ext4;
    ...
    set_opt(sb, INIT_INODE_TABLE);
    if (def_mount_opts & EXT4_DEFM_DEBUG) set_opt(sb, DEBUG);
    if (def_mount_opts & EXT4_DEFM_BSDGROUPS) set_opt(sb, GRPID);
    if (def_mount_opts & EXT4_DEFM_UID16) set_opt(sb, NO_UID32);
    ...
    if (ext4_has_metadata_csum(sb)) set_opt(sb, JOURNAL_CHECKSUM);
    ...
    if (le16_to_cpu(sbi->s_es->s_errors) == EXT4_ERRORS_PANIC) {
        if (!capable(CAP_SYS_ADMIN)) goto failed_mount;
    ...
    if (def_mount_opts & EXT4_DEFM_DISCARD) set_opt(sb, DISCARD);

    sbi->s_resuid = make_kuid(sb->s_user_ns, le16_to_cpu(es->s_def_resuid));
    if (!uid_valid(sbi->s_resuid)) sbi->s_resuid = make_kuid(sb->s_user_ns, EXT4_DEF_RESUID);
    sbi->s_resgid = make_kgid(sb->s_user_ns, le16_to_cpu(es->s_def_resgid));
    if (!gid_valid(sbi->s_resgid)) sbi->s_resgid = make_kgid(sb->s_user_ns, EXT4_DEF_RESGID);
    ...
    if (sbi->s_es->s_mount_opts[0]) {
        char *s_mount_opts =
            kstrndup(sbi->s_es->s_mount_opts, sizeof(sbi->s_es->s_mount_opts), GFP_KERNEL);
        if (!s_mount_opts) goto failed_mount;
    ...
    if (!parse_options((char *)data, sb, &journal_devnum, &journal_ioprio, 0)) goto failed_mount;
    ...
    if (test_opt(sb, DATA_FLAGS) == EXT4_MOUNT_JOURNAL_DATA) {
        ...
        if (test_opt(sb, DELALLOC)) clear_opt(sb, DELALLOC);
    } else {
    ...
    if (le32_to_cpu(es->s_rev_level) == EXT4_GOOD_OLD_REV &&
        (ext4_has_compat_features(sb) || ext4_has_ro_compat_features(sb) ||
    ...
    if (IS_EXT2_SB(sb)) {
        if (ext2_feature_set_ok(sb))
             ...
             */
            if (silent && ext4_feature_set_ok(sb, sb_rdonly(sb))) goto failed_mount;
            ext4_msg(sb, KERN_ERR,
     ...
     */
    if (!ext4_feature_set_ok(sb, (sb_rdonly(sb)))) goto failed_mount;

    blocksize = BLOCK_SIZE << le32_to_cpu(es->s_log_block_size);
    if (blocksize < EXT4_MIN_BLOCK_SIZE || blocksize > EXT4_MAX_BLOCK_SIZE) {
        ext4_msg(sb, KERN_ERR, "Unsupported filesystem blocksize %d (%d log_block_size)", blocksize,
    ...
    if (le32_to_cpu(es->s_rev_level) == EXT4_GOOD_OLD_REV) {
        ...
        if ((sbi->s_inode_size < EXT4_GOOD_OLD_INODE_SIZE) || (!is_power_of_2(sbi->s_inode_size)) ||
    ...
    if (ext4_has_feature_64bit(sb)) {
        if (sbi->s_desc_size < EXT4_MIN_DESC_SIZE_64BIT || sbi->s_desc_size > EXT4_MAX_DESC_SIZE ||
    ...
    if (sbi->s_inodes_per_block == 0) goto cantfind_ext4;
    ...
    for (i = 0; i < 4; i++) sbi->s_hash_seed[i] = le32_to_cpu(es->s_hash_seed[i]);
    ...
    if (ext4_has_feature_dir_index(sb)) {
        ...
        if (i & EXT2_FLAGS_UNSIGNED_HASH)
            ...
            if (!sb_rdonly(sb)) es->s_flags |= cpu_to_le32(EXT2_FLAGS_UNSIGNED_HASH);
            sbi->s_hash_unsigned = 3;
#else
            if (!sb_rdonly(sb)) es->s_flags |= cpu_to_le32(EXT2_FLAGS_SIGNED_HASH);
#endif
        }
    }
    ...
    if (has_bigalloc) {
        ...
        sbi->s_cluster_bits =
            le32_to_cpu(es->s_log_cluster_size) - le32_to_cpu(es->s_log_block_size);
    ...
    if (sbi->s_blocks_per_group == clustersize << 3) set_opt2(sb, STD_GROUP_SIZE);
    ...
    if (err) {
        ...
        if (sizeof(sector_t) < 8) ext4_msg(sb, KERN_WARNING, "CONFIG_LBDAF not enabled");
        goto failed_mount;
    }

    if (EXT4_BLOCKS_PER_GROUP(sb) == 0) goto cantfind_ext4;
    ...
    if (le32_to_cpu(es->s_first_data_block) >= ext4_blocks_count(es)) {
        ext4_msg(sb, KERN_WARNING,
                 ...
                 le32_to_cpu(es->s_first_data_block), ext4_blocks_count(es));
        goto failed_mount;
    }
    ...
    blocks_count = (ext4_blocks_count(es) - le32_to_cpu(es->s_first_data_block) +
    ...
    if (blocks_count > ((uint64_t)1 << 32) - EXT4_DESC_PER_BLOCK(sb)) {
        ext4_msg(sb, KERN_WARNING,
                 ...
                 "blocks per group %lu)",
                 sbi->s_groups_count, ext4_blocks_count(es), le32_to_cpu(es->s_first_data_block),
    ...
    if (((u64)sbi->s_groups_count * sbi->s_inodes_per_group) != le32_to_cpu(es->s_inodes_count)) {
    ...
    if (ext4_es_register_shrinker(sbi)) goto failed_mount3;
    ...
    if (ext4_has_feature_mmp(sb) && !sb_rdonly(sb))
        if (ext4_multi_mount_protect(sb, le64_to_cpu(es->s_mmp_block))) goto failed_mount3a;
    ...
    if (ext4_has_feature_64bit(sb) &&
        !jbd2_journal_set_features(EXT4_SB(sb)->s_journal, 0, 0, JBD2_FEATURE_INCOMPAT_64BIT)) {
    ...
    switch (test_opt(sb, DATA_FLAGS)) {
        case 0:
             ...
             */
            if (jbd2_journal_check_available_features(sbi->s_journal, 0, 0,
...
no_journal:
    ...
    if (!err) err = percpu_counter_init(&sbi->s_dirs_counter, ext4_count_dirs(sb), GFP_KERNEL);
    if (!err) err = percpu_counter_init(&sbi->s_dirtyclusters_counter, 0, GFP_KERNEL);
    if (!err) err = percpu_init_rwsem(&sbi->s_journal_flag_rwsem);
    ...
    if (err) goto failed_mount6;

    err = ext4_register_sysfs(sb);
    if (err) goto failed_mount7;

#ifdef CONFIG_QUOTA
    /* Enable quota usage during mount. */
    if (ext4_has_feature_quota(sb) && !sb_rdonly(sb)) {
        err = ext4_enable_quotas(sb);
        if (err) goto failed_mount8;
    }
#endif /* CONFIG_QUOTA */
    ...
    if (___ratelimit(&ext4_mount_msg_ratelimit, "EXT4-fs mount"))
        ext4_msg(sb, KERN_INFO,
                 ...
                 "Opts: %.*s%s%s",
                 descr, (int)sizeof(sbi->s_es->s_mount_opts), sbi->s_es->s_mount_opts,
                 *sbi->s_es->s_mount_opts ? "; " : "", orig_data);

    if (es->s_error_count) mod_timer(&sbi->s_err_report, jiffies + 300 * HZ); /* 5 minutes */
    ...
    // for mqfs
    struct init_param param = {.nr_device = 1,
                               .cmb_device_id = disk_devt(sbi->s_sb->s_bdev->bd_disk),
                               .parea_partition_hint = sbi->s_blocks_per_group};
    param.device_ids = kzalloc(sizeof(dev_t) * 1, GFP_KERNEL);
    param.sbs = kzalloc(sizeof(struct super_block *) * 1, GFP_KERNEL);
    for (i = 0; i < 1; i++) {  // TODO: currently only support 1 device
        param.device_ids[i] = disk_devt(sbi->s_sb->s_bdev->bd_disk);
        param.sbs[i] = sb;
    }

    if (sbi->crashmonkey) // force recovery, let crashmonkey understand ccnvme
        sbi->nr_streams = 2;

    if (sbi->nr_streams <=0 )
        sbi->nr_streams = num_possible_cpus();

    sbi->streams = kzalloc(sizeof(u32) * sbi->nr_streams, GFP_KERNEL);
    olayer_register_streams(sbi->nr_streams, sbi->streams, &param);
                         
    if (sbi->crashmonkey && sbi->nr_streams != param.registered) { // force recovery checking
        olayer_crash_recovery(sbi->s_sb->s_bdev); // recovery from P-SQ
    }
    sbi->nr_streams = param.registered;

    pr_info("mqfs mount with %u streams\n", sbi->nr_streams);
    sbi->stream_lock = kmalloc(sbi->nr_streams * sizeof(struct mutex), GFP_KERNEL);
    for (i = 0; i < sbi->nr_streams; i++) mutex_init(&sbi->stream_lock[i]);

    atomic_set(&sbi->txid_alloc, 0);
    kfree(param.device_ids);
    kfree(param.sbs);

#ifdef PERF_DECOMPOSE
    reset_perf_counters(sbi);
#endif
...
cantfind_ext4:
    if (!silent) ext4_msg(sb, KERN_ERR, "VFS: Can't find ext4 filesystem");
    goto failed_mount;
...
failed_mount6:
    ext4_mb_release(sb);
    if (sbi->s_flex_groups) kvfree(sbi->s_flex_groups);
    percpu_counter_destroy(&sbi->s_freeclusters_counter);
    percpu_counter_destroy(&sbi->s_freeinodes_counter);
    percpu_counter_destroy(&sbi->s_dirs_counter);
    percpu_counter_destroy(&sbi->s_dirtyclusters_counter);
...
failed_mount4:
    ...
    if (EXT4_SB(sb)->rsv_conversion_wq) destroy_workqueue(EXT4_SB(sb)->rsv_conversion_wq);
failed_mount_wq:
    if (sbi->s_ea_inode_cache) {
        ext4_xattr_destroy_cache(sbi->s_ea_inode_cache);
        sbi->s_ea_inode_cache = NULL;
...
failed_mount:
    if (sbi->s_chksum_driver) crypto_free_shash(sbi->s_chksum_driver);
#ifdef CONFIG_QUOTA
    for (i = 0; i < EXT4_MAXQUOTAS; i++) kfree(sbi->s_qf_names[i]);
#endif
    ext4_blkdev_remove(sbi);
    brelse(bh);
out_fail:
...
static journal_t *ext4_get_dev_journal(struct super_block *sb, dev_t j_dev) {
    ...
    if ((le16_to_cpu(es->s_magic) != EXT4_SUPER_MAGIC) ||
        !(le32_to_cpu(es->s_feature_incompat) & EXT4_FEATURE_INCOMPAT_JOURNAL_DEV)) {
        ext4_msg(sb, KERN_ERR,
...
static int ext4_load_journal(struct super_block *sb, struct ext4_super_block *es,
                             unsigned long journal_devnum) {
    journal_t *journal;
    ...
    if (journal_inum) {
        if (!(journal = ext4_get_journal(sb, journal_inum))) return -EINVAL;
    } else {
        if (!(journal = ext4_get_dev_journal(sb, journal_dev))) return -EINVAL;
    }

    if (!(journal->j_flags & JBD2_BARRIER)) ext4_msg(sb, KERN_INFO, "barriers disabled");

    if (!ext4_has_feature_journal_needs_recovery(sb))
        err = jbd2_journal_wipe(journal, !really_read_only);
    if (!err) {
        char *save = kmalloc(EXT4_S_ERR_LEN, GFP_KERNEL);
        if (save) memcpy(save, ((char *)es) + EXT4_S_ERR_START, EXT4_S_ERR_LEN);
        err = jbd2_journal_load(journal);
        if (save) memcpy(((char *)es) + EXT4_S_ERR_START, save, EXT4_S_ERR_LEN);
        kfree(save);
    }
...
static int ext4_commit_super(struct super_block *sb, int sync) {
    ...
    if (!sbh || block_device_ejected(sb)) return error;

    /*
     * The superblock bh should be mapped, but it might not be if the
     * device was hot-removed. Not much we can do but fail the I/O.
     */
    if (!buffer_mapped(sbh)) return error;
     ...
     */
    if (!(sb->s_flags & SB_RDONLY)) es->s_wtime = cpu_to_le32(get_seconds());
    if (sb->s_bdev->bd_part)
    ...
    if (percpu_counter_initialized(&EXT4_SB(sb)->s_freeclusters_counter))
        ext4_free_blocks_count_set(
            es, EXT4_C2B(EXT4_SB(sb),
                         percpu_counter_sum_positive(&EXT4_SB(sb)->s_freeclusters_counter)));
    if (percpu_counter_initialized(&EXT4_SB(sb)->s_freeinodes_counter))
        es->s_free_inodes_count =
            cpu_to_le32(percpu_counter_sum_positive(&EXT4_SB(sb)->s_freeinodes_counter));
    BUFFER_TRACE(sbh, "marking dirty");
    ext4_superblock_csum_set(sb);
    if (sync) lock_buffer(sbh);
...
static void ext4_mark_recovery_complete(struct super_block *sb, struct ext4_super_block *es) {
    ...
    if (jbd2_journal_flush(journal) < 0) goto out;
...
static int ext4_sync_fs(struct super_block *sb, int wait) {
    ...
    if (unlikely(ext4_forced_shutdown(sbi))) return 0;
    ...
    if (sbi->s_journal) {
        ...
        if (jbd2_journal_start_commit(sbi->s_journal, &target)) {
            if (wait) ret = jbd2_log_wait_commit(sbi->s_journal, target);
        }
...
out:
    if (journal) /* we rely on upper layer to stop further updates */
        jbd2_journal_unlock_updates(journal);
    return error;
}
 ...
 */
static int ext4_unfreeze(struct super_block *sb) {
    if (sb_rdonly(sb) || ext4_forced_shutdown(EXT4_SB(sb))) return 0;
...
static int ext4_remount(struct super_block *sb, int *flags, char *data) {
    ...
    for (i = 0; i < EXT4_MAXQUOTAS; i++)
        if (sbi->s_qf_names[i]) {
            ...
            if (!old_opts.s_qf_names[i]) {
                for (j = 0; j < i; j++) kfree(old_opts.s_qf_names[j]);
                kfree(orig_data);
                return -ENOMEM;
            }
        } else
    ...
    if (sbi->s_mount_flags & EXT4_MF_FS_ABORTED) ext4_abort(sb, "Abort forced by user");
    ...
    if (*flags & SB_LAZYTIME) sb->s_flags |= SB_LAZYTIME;
    ...
    if ((bool)(*flags & SB_RDONLY) != sb_rdonly(sb)) {
        ...
        if (*flags & SB_RDONLY) {
            ...
            if (sbi->s_journal) ext4_mark_recovery_complete(sb, es);
            if (sbi->s_mmp_tsk) kthread_stop(sbi->s_mmp_tsk);
        } else {
            ...
            for (g = 0; g < sbi->s_groups_count; g++) {
                ...
                if (!ext4_group_desc_csum_verify(sb, g, gdp)) {
                    ext4_msg(sb, KERN_ERR, "ext4_remount: Checksum for group %u failed (%u!=%u)", g,
                             le16_to_cpu(ext4_group_desc_csum(sb, g, gdp)),
                             le16_to_cpu(gdp->bg_checksum));
                    err = -EFSBADCRC;
                    goto restore_opts;
                }
             ...
             * the partition.)
             */
            if (sbi->s_journal) ext4_clear_journal_err(sb, es);
            sbi->s_mount_state = le16_to_cpu(es->s_state);

            err = ext4_setup_super(sb, es, 0);
            if (err) goto restore_opts;
    ...
    for (i = 0; i < EXT4_MAXQUOTAS; i++) kfree(old_opts.s_qf_names[i]);
    if (enable_quota) {
...
restore_opts:
    ...
    synchronize_rcu();
    for (i = 0; i < EXT4_MAXQUOTAS; i++) kfree(to_free[i]);
#endif
    kfree(orig_data);
    return err;
}
...
static int ext4_statfs_project(struct super_block *sb, kprojid_t projid, struct kstatfs *buf) {
    ...
    dquot = dqget(sb, qid);
    if (IS_ERR(dquot)) return PTR_ERR(dquot);
    ...
    if (limit && buf->f_blocks > limit) {
        curblock =
            (dquot->dq_dqb.dqb_curspace + dquot->dq_dqb.dqb_rsvspace) >> sb->s_blocksize_bits;
        buf->f_blocks = limit;
        buf->f_bfree = buf->f_bavail = (buf->f_blocks > curblock) ? (buf->f_blocks - curblock) : 0;
    }

    limit =
        dquot->dq_dqb.dqb_isoftlimit ? dquot->dq_dqb.dqb_isoftlimit : dquot->dq_dqb.dqb_ihardlimit;
...
static int ext4_statfs(struct dentry *dentry, struct kstatfs *buf) {
    ...
    if (!test_opt(sb, MINIX_DF)) overhead = sbi->s_overhead;
    ...
    if (buf->f_bfree < (ext4_r_blocks_count(es) + resv_blocks)) buf->f_bavail = 0;
 ...
 */
static int ext4_quota_on_mount(struct super_block *sb, int type) {
    return dquot_quota_on_mount(sb, get_qf_name(sb, EXT4_SB(sb), type), EXT4_SB(sb)->s_jquota_fmt,
                                type);
}
...
static int ext4_quota_on(struct super_block *sb, int type, int format_id, const struct path *path) {
    ...
    if (path->dentry->d_sb != sb) return -EXDEV;
    /* Journaling quota? */
    ...
    if (err) {
        ...
        if (IS_ERR(handle)) goto unlock_inode;
...
static int ext4_quota_enable(struct super_block *sb, int type, int format_id, unsigned int flags) {
    ...
    unsigned long qf_inums[EXT4_MAXQUOTAS] = {le32_to_cpu(EXT4_SB(sb)->s_es->s_usr_quota_inum),
                                              le32_to_cpu(EXT4_SB(sb)->s_es->s_grp_quota_inum),
                                              le32_to_cpu(EXT4_SB(sb)->s_es->s_prj_quota_inum)};

    BUG_ON(!ext4_has_feature_quota(sb));

    if (!qf_inums[type]) return -EPERM;
    ...
    iput(qf_inode);
    if (err) lockdep_set_quota_inode(qf_inode, I_DATA_SEM_NORMAL);
...
static int ext4_enable_quotas(struct super_block *sb) {
    ...
    for (type = 0; type < EXT4_MAXQUOTAS; type++) {
        if (qf_inums[type]) {
            err = ext4_quota_enable(
                sb, type, QFMT_VFS_V1,
                DQUOT_USAGE_ENABLED | (quota_mopt[type] ? DQUOT_LIMITS_ENABLED : 0));
            if (err) {
                for (type--; type >= 0; type--) dquot_quota_off(sb, type);
...
static int ext4_quota_off(struct super_block *sb, int type) {
    ...
    if (test_opt(sb, DELALLOC)) sync_filesystem(sb);

    if (!inode || !igrab(inode)) goto out;

    err = dquot_quota_off(sb, type);
    if (err || ext4_has_feature_quota(sb)) goto out_put;
     ...
     */
    handle = ext4_journal_start(inode, EXT4_HT_QUOTA, 1);
    if (IS_ERR(handle)) goto out_unlock;
    EXT4_I(inode)->i_flags &= ~(EXT4_NOATIME_FL | EXT4_IMMUTABLE_FL);
...
static ssize_t ext4_quota_read(struct super_block *sb, int type, char *data, size_t len,
    ...
    if (off + len > i_size) len = i_size - off;
    toread = len;
    while (toread > 0) {
...
static ssize_t ext4_quota_write(struct super_block *sb, int type, const char *data, size_t len,
    ...
    do {
        bh = ext4_bread(handle, inode, blk,
                        EXT4_GET_BLOCKS_CREATE | EXT4_GET_BLOCKS_METADATA_NOFAIL);
...
static int ext4_get_next_id(struct super_block *sb, struct kqid *qid) {
    ...
    if (!sb_has_quota_loaded(sb, qid->type)) return -ESRCH;
    ops = sb_dqopt(sb)->ops[qid->type];
    if (!ops || !ops->get_next_id) return -ENOSYS;
...
static inline void register_as_ext2(void) {
    ...
    if (err) printk(KERN_WARNING "EXT4-fs: Unable to register as ext2 (%d)\n", err);
}

static inline void unregister_as_ext2(void) { unregister_filesystem(&ext2_fs_type); }

static inline int ext2_feature_set_ok(struct super_block *sb) {
    if (ext4_has_unknown_ext2_incompat_features(sb)) return 0;
    if (sb_rdonly(sb)) return 1;
    if (ext4_has_unknown_ext2_ro_compat_features(sb)) return 0;
    return 1;
}
#else
static inline void register_as_ext2(void) {}
...
static inline void register_as_ext3(void) {
    ...
    if (err) printk(KERN_WARNING "EXT4-fs: Unable to register as ext3 (%d)\n", err);
}

static inline void unregister_as_ext3(void) { unregister_filesystem(&ext3_fs_type); }

static inline int ext3_feature_set_ok(struct super_block *sb) {
    if (ext4_has_unknown_ext3_incompat_features(sb)) return 0;
    if (!ext4_has_feature_journal(sb)) return 0;
    if (sb_rdonly(sb)) return 1;
    if (ext4_has_unknown_ext3_ro_compat_features(sb)) return 0;
    return 1;
}
...
static int __init ext4_init_fs(void) {
    ...
    for (i = 0; i < EXT4_WQ_HASH_SZ; i++) init_waitqueue_head(&ext4__ioend_wq[i]);
...
module_init(ext4_init_fs) module_exit(ext4_exit_fs)
