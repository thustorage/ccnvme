...
enum mqfs_state_bits {
	BH_Overwrite = BH_JBDPrivateStart,
};
BUFFER_FNS(Overwrite, overwrite);

#if 0
#define MQFS_DEBUG(fmt, ...)                                                                               \
  do {                                                                                             \
...
// #define PERF_DECOMPOSE
// #define DISABLE_METAPAGING

#ifdef PERF_DECOMPOSE
#define record_time(fn, ret, counter) \
do {	\
	u64 s = ktime_get_ns(); \
	ret = fn;	\
	u64 e = ktime_get_ns();	\
...
#ifdef PERF_DECOMPOSE
#define reset_perf_counters(sbi) do { \
    sbi->sync_file_count = sbi->sync_parent_count = sbi->file_submit_time = \
    sbi->parent_submit_time = sbi->file_wait_time = sbi->parent_wait_time = \
    sbi->fdata_submit_time = sbi->finode_submit_time = sbi->fmapping_submit_time = \
	sbi->fdata_wait_time = sbi->finode_wait_time = sbi->fmapping_wait_time = \
	sbi->tx_write_time = sbi->tx_begin_time = sbi->tx_end_time = sbi->tx_write_count = \
	sbi->fjournal_des_submit_time = sbi->fjournal_des_wait_time = 0; \
} while(0)
#endif
...
struct ext4_inode_info {
	...
	// mqfs 
	struct list_head fsmeta_pending_list;
	struct list_head inodemeta_pending_list;
	struct list_head copyout_inode_list;
};
...
struct ext4_sb_info {
	...
	// mqfs for performance decomposing
#ifdef PERF_DECOMPOSE
	u64 sync_file_count;
	u64 sync_parent_count;

	u64 file_submit_time;
	u64 file_wait_time;

	u64 fdata_submit_time;
	u64 fmapping_submit_time;
	u64 finode_submit_time;
	u64 fjournal_des_submit_time;

	u64 fdata_wait_time;
	u64 fmapping_wait_time;
	u64 finode_wait_time;
	u64 fjournal_des_wait_time;

	u64 parent_submit_time;
	u64 parent_wait_time;
...
#ifdef FATOMIC_FILE
extern int ext4_atomic_file(struct file *, loff_t, loff_t, int);
#endif
// for mqfs
/* filemap_compat.c */
int __generic_file_atomic_write(struct file *file, loff_t start, loff_t end, int datasync, uint txid);
int __generic_file_atomic_write_wait(struct file *file, loff_t start, loff_t end, int datasync);
int __generic_file_atomic_write_wait_nodata(struct file *file, loff_t start, loff_t end, int datasync);
int __generic_file_atomic_write_submit(struct file *file, loff_t start, loff_t end, int datasync,uint stream);

int mqfs_atomic_write_parent_submit(struct inode *inode, uint stream, struct list_head *waiting_list);
