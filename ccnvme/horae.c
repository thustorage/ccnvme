...
void NVM_FLUSH(u64 start, u64 size) {
    if (nvm_flush_lat <= 0) return;
    int cacheline = size % CACHELINE_SIZE == 0 ? size / CACHELINE_SIZE : size / CACHELINE_SIZE + 1;
    ndelay(cacheline * nvm_flush_lat);
}
#endif

unsigned int version_consistency = 0;
module_param(version_consistency, uint, 0444);
MODULE_PARM_DESC(version_consistency, "Version consistency enabled. Default 0, disabled.");
...
static int stream_tx_commit_core(struct stream *stream, uint flags, u64 txid) {
    ...
    cmb_flush(&ci, stream, stream->last_queue_flushed * sizeof(struct nvme_rw_command_),
              sizeof(struct nvme_rw_command_) * nr_q_entries);
...
retry:
    ...
    cmb_flush(&ci, stream, tail_old * sizeof(struct nvme_rw_command_),
              1 * sizeof(struct nvme_rw_command_));
#endif
...
void stream_submit_bio_ipu(struct bio *bio, uint sid, int forced) {
    ...
    olayer_debug(3, " to device %lu, lba %llu, len (%llu/%llu)\n", device_id, lba, nr_pages,
                 bio->bi_iter.bi_size);
    if (version_consistency || forced) {
        plba = journal_write_alloc(stream, bio);  
        if (plba < 0) {
            submit_bio(bio);
        } else {
...
#ifdef USE_CMB
int write_cmb_test(int unit, int sync, uint pos) {  // unit in 8 Bytes
    struct stream *stream;
    void *start;
...
int write_cmb_test_multithread(int unit, int sync, uint pos, uint sid) {  // unit in 8 Bytes
    struct stream *stream;
    void *start;
...
EXPORT_SYMBOL(write_cmb_test_multithread);

int read_cmb_test(int unit, uint pos) {  // unit in 8 Bytes
    struct stream *stream;
...
int read_cmb_test_multithread(int unit, uint pos, uint sid) {  // unit in 8 Bytes
    struct stream *stream;
...
EXPORT_SYMBOL(read_cmb_test_multithread);
#endif
