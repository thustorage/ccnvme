			     ...
			     struct nvm_geo *geo)
{
	...
	geo->ws_opt = sec_per_pg*3; //bookmark: set for TLC NAND
	...
	printk("ocssd[%s]: sec_per_pg=%d, mw_cunits=%d, ws_opt=%d, csecs=%d, all_luns=%d\n", __func__, sec_per_pg, geo->mw_cunits, geo->ws_opt, geo->csecs, geo->all_luns);
...
int nvme_nvm_register_sysfs(struct nvme_ns *ns)
{
	...
	printk("ocssd[%s]: lightnvm ver:%d\n", __func__, geo->major_ver_id);
